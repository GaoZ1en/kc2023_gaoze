...
void fitDiLc_Gaussian()
{
   gROOT->ProcessLine(".x /afs/ihep.ac.cn/users/g/gaoze/private/workfs/DiLc/datafiles/lhcbStyle.C");

   TChain* newtree = new TChain();
   newtree->Add("/afs/ihep.ac.cn/users/g/gaoze/private/workfs/DiLc/datafiles/XiccWSTuple.root/DecayTree");

   RooArgSet vars("Variables");
   RooRealVar m1("Lc_M","M(#Lambda_{c}^{+}_{1}#Lambda_{c}^{-}_{1})",2220,2360,"MeV/c^{2}");
   RooRealVar m2("LcBar_M","M(#Lambda_{c}^{+}_{2}#Lambda_{c}^{-}_{2})",2220,2360,"MeV/c^{2}");
   vars.add(m1);
   vars.add(m2);
   ...
   //RooRealVar nl("nl","nl",8,0,15);
   //RooRealVar nr("nr","nr",8,0,15);
   ...
   //************************* sweight ***************************
   TFile* file = new TFile("XiccWSTuple_sw.root","recreate"); 
