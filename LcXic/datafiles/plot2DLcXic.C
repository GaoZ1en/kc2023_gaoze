...
void plot2DLcXic()
{
   ...
   h_2D->SetFillColor(kLightTemperature);
   ...
   h_2D->GetYaxis()->SetTitle("#it{M}(#it{#bar{#Xi_{c}^{#minus}}}) [MeV/c^{2}]");
   ...
   c_2D->SaveAs("fitplot3D.pdf");
   c_2D->SaveAs("fitplot3D.png");
   c_2D->SaveAs("fitplot3D.eps");
} 
