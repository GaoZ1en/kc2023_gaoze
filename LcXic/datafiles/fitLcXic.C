...
void fitLcXic()
{
   gROOT->ProcessLine(".x /afs/ihep.ac.cn/users/g/gaoze/private/workfs/LcXic/datafiles/lhcbStyle.C");

   TChain* chain = new TChain();
   chain->Add("/afs/ihep.ac.cn/users/g/gaoze/private/workfs/LcXic/datafiles/XiccTuple.root/DecayTree");
   TTree* newtree = chain->CopyTree("");

   RooArgSet vars("Variables");
   RooRealVar m_Lc("Lc_M","M(#Lambda_{c}^{+})",2210,2360,"MeV/c^{2}");
   RooRealVar m_Xic("Xic_M","M(#bar{#Xi_{c}^{-}})",2390,2540,"MeV/c^{2}");
   vars.add(m_Lc);
   vars.add(m_Xic);
   ...
   RooRealVar mean_Xic("#mu_Xic","mean_Xic",2467,2390,2540);
   ...
   //c_Xic->SaveAs("fitLcBar.png");
   //c_Xic->SaveAs("fitLcBar.eps");
   //c_Xic->SaveAs("fitLcBar.pdf");
   ...
   for(int i=0;i<newtree->GetEntries();i++){
      newtree->GetEntry(i);
	if(!(t.Lc_M>2210&&t.Lc_M<2360&&t.Xic_M>2390&&t.Xic_M<2540)) continue;
	j += 1;
