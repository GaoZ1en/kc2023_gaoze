...
void fitDiLc_res()
{
   ...
   tree->Project("hRS","C_M-Lc_M-LcBar_M+2286.99*2");
   ...
   RooRealVar *mean2 = new RooRealVar("#mu2","mean2",4598.77,4575,4625);
   RooRealVar *width2 = new RooRealVar("#width2","width2",33.91,0,50);
   ...
   RooRealVar *mean3 = new RooRealVar("#mu3","mean3",4661.88,4625,4675);
   RooRealVar *width3 = new RooRealVar("#width3","width3",107.4,0,150);
   ...
   RooRealVar *mean4 = new RooRealVar("#mu4","mean4",4763.74,4740,4840);
   RooRealVar *width4 = new RooRealVar("#width4","width4",68.78,0,120);
   ...
   RooRealVar *mean5 = new RooRealVar("#mu5","mean5",4900.55,4850,4950);
   RooRealVar *width5 = new RooRealVar("#width5","width5",44.93,0,200);
   ...
   RooRealVar *width6 = new RooRealVar("#width6","width6",6.84,1,50);
   ...
   RooRealVar *mean7 = new RooRealVar("#mu7","mean7",5025.62,4950,5050);
   RooRealVar *width7 = new RooRealVar("#width7","width7",137.6,0,150);
   ...
   // **** background ****
   RooRealVar *BkgMSlope = new RooRealVar("c_{exp}","Slope of mass bkg",-0.000536258,-1.0,1.0);
   BkgMSlope->setConstant(true);
   RooAbsPdf  *bkg = new RooExponential("bkg","bkg", *m, *BkgMSlope);

   TH1F *h_sb = new TH1F("h_sb","h_sb",75,4575,6075);
   TChain *chain_sb = new TChain("DecayTree");
   chain_sb->AddFile("/afs/ihep.ac.cn/users/g/gaoze/private/workfs/DiLc/datafiles/XiccTuple_sw.root");
   TTree *tree_sb = chain->CopyTree(LcCut);
   tree_sb->Project("h_sb","C_MixM-Lc_M-LcBar_M+2286.99*2","nbs_sw");
   RooDataHist *data_sb = new RooDataHist("data_sb","dataset with m", *m, h_sb);
   RooHistPdf *bkg_sb = new RooHistPdf("bkg_sb","bkg_sb",*m,*data_sb);

   RooRealVar *Nsig2 = new RooRealVar("nsig2","Number of signal events",1765.56,0,10000);
   RooRealVar *Nsig3 = new RooRealVar("nsig3","Number of signal events",3443.46,0,10000);
   RooRealVar *Nsig4 = new RooRealVar("nsig4","Number of signal events",325.097,0,10000);
   RooRealVar *Nsig5 = new RooRealVar("nsig5","Number of signal events",277.264,0,10000);
   RooRealVar *Nsig6 = new RooRealVar("nsig6","Number of signal events",42.3962,0,10000);
   RooRealVar *Nsig7 = new RooRealVar("nsig7","Number of signal events",484.336,0,10000);
   RooRealVar *Nbkg = new RooRealVar("nbkg","Number of bkg",1401,0,10000);
   RooRealVar *Nbkg_sb = new RooRealVar("nbkg_sb","Number of bkg_sb",14888,0,20000);

   // **** total pdf and fit ****
   RooAbsPdf *total= new RooAddPdf("total","sum of signal and background PDF's",RooArgList(*sig2,*sig3,*sig4,*sig5,*sig6,*sig7,*bkg,*bkg_sb),RooArgList(*Nsig2,*Nsig3,*Nsig4,*Nsig5,*Nsig6,*Nsig7,*Nbkg,*Nbkg_sb));
   ...
   total->plotOn(mframe,Components(*bkg_sb),LineColor(kBlue+1),LineStyle(kDashed),Name("sideband background"));
   ...
   TLegend *legend = new TLegend(0.63,0.4,0.83,0.9);
   legend->SetFillColor(kWhite);
   legend->SetLineColor(kWhite);
   legend->SetTextSize(0.04);
   legend->AddEntry("Data","Data");
   legend->AddEntry("total fit","Total fit","l");
   legend->AddEntry("1?"        ,"unknown state 1","l");
   legend->AddEntry("#psi(4660)","#psi(4660)","l");
   legend->AddEntry("#psi(4790)","#psi(4790)","l");
   legend->AddEntry("#psi(4900)","#psi(4900)","l");
   legend->AddEntry("B^{0}_{s}" ,"B_{s}","l");
   legend->AddEntry("2?"        ,"unknown state 2","l");
   legend->AddEntry("WS background","WS background","l");
   legend->AddEntry("sideband background","sideband background","l");
   legend->Draw();
