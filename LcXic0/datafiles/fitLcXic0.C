...
void fitLcXic0()
{
   ...
   TCut *Xic0Cut = new TCut("Xic_M>2400 && Xic_M<2550");

   TChain* chain = new TChain();
   chain->Add("/home/Koishi/LcXic0/datafiles/XiccTuple.root/DecayTree");
   TTree* newtree = chain->CopyTree("");

   RooArgSet vars("Variables");
   RooRealVar m_Lc("Lc_M","M(#Lambda_{c}^{+})",2210,2360,"MeV/c^{2}");
   RooRealVar m_Xic0("Xic_M","M(#Xi_{c}^{0})",2400,2550,"MeV/c^{2}");
   vars.add(m_Lc);
   vars.add(m_Xic0);
   ...
   RooRealVar sigma_Lc("#sigma_Lc","sigma_Lc",6,2.,20);
   //RooRealVar alphal_Lc("al_Lc","alphal_Lc",1,0,5);
   //RooRealVar alphar_Lc("ar_Lc","alphar_Lc",1,0,5);
   //RooRealVar nl_Lc("nl_Lc","nl_Lc",5,0,10);
   //RooRealVar nr_Lc("nr_Lc","nr_Lc",5,0,10);
   ...
   //RooGaussian sig_Lc1("sig_Lc1","sig_Lc1",m_Xic0,mean_Lc,sigma_Lc1);
   //RooGaussian sig_Lc2("sig_Lc2","sig_Lc2",m_Xic0,mean_Lc,sigma_Lc2);
   //RooAddPdf sig_Lc("sig_Lc","sig_Lc",RooArgList(sig_Lc1,sig_Lc2),RooArgList(fsig_Lc));

   // #### sig_Xic ####
   RooRealVar mean_Xic("#mu_Xic","mean_Xic",2470.44,2450,2490);
   ...
   //MyCB sig_Xic("sig_Xic","sig_Xic",m_Xic0,mean_Xic,sigma_Xic,alphal_Xic,nl_Xic,alphar_Xic,nr_Xic);
   ...
   RooCBShape sig_Xic("sig_Xic","sig_Xic",m_Xic0,mean_Xic,sigma_Xic,alphal_Xic,alphar_Xic);
   ...
   //RooGaussian sig_Xic1("sig_Xic1","sig_Xic1",m_Xic0,mean_Xic,sigma_Xic1);
   //RooGaussian sig_Xic2("sig_Xic2","sig_Xic2",m_Xic0,mean_Xic,sigma_Xic2);
   //RooAddPdf sig_Xic("sig_Xic","sig_Xic",RooArgList(sig_Xic1,sig_Xic2),RooArgList(fsig_Xic));
   
   //RooRealVar sigma_Xic("#sigma_Xic","sigma_Xic",6,0.,20);
   //RooGaussian sig_Xic("sig_Xic","sig_Xic",m_Xic0,mean_Xic,sigma_Xic);
   ...
   RooExponential combbkga_Xic("exponentiala_Xic","exponential backgrounda1_Xic",m_Xic0,taua_Xic);   
   ...
   RooExponential combbkgb_Xic("exponentialb_Xic","exponential backgroundb2_Xic",m_Xic0,taub_Xic);
   ...
   RooRealVar Nsig("nsig","Number of signal events",600,0,1000);
   RooRealVar Nbkg("nbs","",100,0,1000);
   RooRealVar Ncombbkg("nbb","",100,0,1000);
   ...
   params->writeToFile("params_sPlot_LcXic0.txt");
   ...
   total.plotOn(m_Lc_frame,Components(sig),LineColor(kRed),LineStyle(kDashed),Name("#Lambda_{c}sig+#Xi^{0}_{c}sig"));
   total.plotOn(m_Lc_frame,Components(sig),FillColor(kRed),FillStyle(3013),DrawOption("F"),Name("#Lambda_{c}sig+#Xi^{0}_{c}sig fill"));
   total.plotOn(m_Lc_frame,Components(bkg1),LineColor(kGreen),LineStyle(kDashed),Name("#Lambda_{c}bkg+#Xi^{0}_{c}sig"));
   total.plotOn(m_Lc_frame,Components(bkg2),LineColor(kGreen+1),LineStyle(kDashed),Name("#Lambda_{c}sig+#Xi^{0}_{c}bkg"));
   total.plotOn(m_Lc_frame,Components(bkg),LineColor(kBlue),LineStyle(kDashed),FillColor(kBlue),Name("#Lambda_{c}bkg+#Xi^{0}_{c}bkg"));
   total.plotOn(m_Lc_frame,Components(bkg),FillColor(kBlue),FillStyle(3013),DrawOption("F"),Name("#Lambda_{c}bkg+#Xi^{0}_{c}bkg fill"));
   ...
   legend->AddEntry("#Lambda_{c}sig+#Xi^{0}_{c}sig","#Lambda_{c}^{+}+#Xi_{c}^{0}","l");
   legend->AddEntry("#Lambda_{c}bkg+#Xi^{0}_{c}sig","bkg_{1}+#Xi_{c}^{0}","l");
   legend->AddEntry("#Lambda_{c}sig+#Xi^{0}_{c}bkg","#Lambda_{c}^{+}+bkg_{2}","l");
   legend->AddEntry("#Lambda_{c}bkg+#Xi^{0}_{c}bkg","bkg_{1}+bkg_{2}","l");
   legend->Draw();
   ...
   // #### Xic0 ####
   RooPlot *m_Xic0_frame=m_Xic0.frame(Title("#bar{#Xi_{c}^{-}} mass fit"));
   data.plotOn(m_Xic0_frame,Binning(100));

   total.plotOn(m_Xic0_frame,Components(sig),LineColor(kRed),LineStyle(kDashed));
   total.plotOn(m_Xic0_frame,Components(sig),FillColor(kRed),FillStyle(3013),DrawOption("F"));
   total.plotOn(m_Xic0_frame,Components(bkg1),LineColor(kGreen),LineStyle(kDashed));
   total.plotOn(m_Xic0_frame,Components(bkg2),LineColor(kGreen+1),LineStyle(kDashed));
   total.plotOn(m_Xic0_frame,Components(bkg),LineColor(kBlue),LineStyle(kDashed));
   total.plotOn(m_Xic0_frame,Components(bkg),FillColor(kBlue),FillStyle(3013),DrawOption("F"));
   total.plotOn(m_Xic0_frame); 
   //total.paramOn(m_Xic0_frame,Layout(0.70,0.95,0.97));
   //m_Xic0_frame->getAttText()->SetTextSize(0.04);

   chi2 =  m_Xic0_frame->chiSquare(7);
   cout << " fitting chi square is : " << chi2 << endl;

   //texchitwo = new TLatex(0.2,0.8,TString::Format("#chi^{2}/ndf = %f",chi2));
   //texchitwo->SetNDC();
   //m_Xic0_frame->addObject(texchitwo);

   TCanvas *c_Xic0 = new TCanvas("c_Xic0","fitted #Xic_{c}^{0} mass",canvasWidth,canvasHeight);
   c_Xic0->SetFillColor(kWhite);
   c_Xic0->Divide(1,2,0,0,0);

   c_Xic0->cd(2);
   gPad->SetTopMargin(0);
   gPad->SetLeftMargin(0.12);
   gPad->SetPad(0.02,0.02,0.98,0.77);
   m_Xic0_frame->GetYaxis()->SetTitleOffset(0.85);
   m_Xic0_frame->Draw();
   ...
   gPad->SetLeftMargin(0.12);
   gPad->SetPad(0.02,0.76,0.98,0.97);
   RooHist* hpull_Xic0 = m_Xic0_frame->pullHist();
   RooPlot* m_Xic0_frame2 = m_Xic0.frame(Title("Pull distribution"));
   hpull_Xic0->SetFillColor(15);
   hpull_Xic0->SetFillStyle(3002);
   m_Xic0_frame2->addPlotable(hpull_Xic0,"L3");
   m_Xic0_frame2->GetYaxis()->SetNdivisions(505);
   m_Xic0_frame2->GetYaxis()->SetLabelSize(0.20);
   m_Xic0_frame2->SetMinimum(-5.0);
   m_Xic0_frame2->SetMaximum(5.0);
   m_Xic0_frame2->Draw();

   c_Xic0->SaveAs("fitXic0.png");
   c_Xic0->SaveAs("fitXic0.eps");
   c_Xic0->SaveAs("fitXic0.pdf");
   ...
   LcXic0_Class t(newtree);

   cout<<"************ check number **************"<<endl;
   cout<<newtree->GetEntries("Lc_M>2210&&Lc_M<2360&&Xic_M>2400&&Xic_M<2550")<<endl;
   cout<<data.numEntries()<<endl;

   int j = -1;
   for(int i=0;i<newtree->GetEntries();i++){
      newtree->GetEntry(i);
	if(!(t.Lc_M>2210&&t.Lc_M<2360&&t.Xic_M>2400&&t.Xic_M<2550)) continue;
	j += 1;
